"""
Script that simulates ancestry for further analysis using msprime with admixture, bottlenecks and splits of populations
Usage:
    python msprime_run.py \
...
parser.add_argument("-bs", "--strength", help="the strength of the bottleneck", type=float, default=100)
...
proportions=[random.randint(1,10) for i in range(anc)]
proportions = [proportions[i]/sum(proportions) for i in range(anc)]


strength=args.strength
demography.add_instantaneous_bottleneck(time=50, strength=strength, population="pop1")
...
ts = msprime.sim_ancestry(popdict,
                          ...
                          sequence_length=args.length,   ##100_000_000, ### !!!!!!!!!!!!!!!!!1 PAVLOS PAVLOS I made this 100,000,000 it was 10 times smaller
...
mts = msprime.sim_mutations(ts, rate=1e-8, random_seed=29324)
