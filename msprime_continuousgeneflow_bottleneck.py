...
"""
Script that simulates mutations and ancestry in the presence of gene flow and bottleneck events
Usage:
    python msprime_run_migration.py \
        ...
        [--bottleneckTime <time_of_bottleneck>]
...
parser.add_argument("-bt", "--bottleneckTime", help="When bottleneck took place", type=int, default=1)
...
for i in range(anc):
    demography.add_population(name="pop"+str(i+1), initial_size=100, initially_active = True)
...
demography.add_population(name="R1", initial_size=args.Nothers, initially_active = True)
demography.add_population(name="R1a", initial_size=args.Nothers, initially_active = True)
demography.add_population(name="R3a", initial_size=args.Nothers, initially_active = True)
demography.add_population(name="R2", initial_size=args.Nothers, initially_active = True)
demography.add_population(name="R3", initial_size=args.Nothers, initially_active = True)
demography.add_population(name="O", initial_size=args.Nothers, initially_active = True)
...
geneflowtime=1
allpopulations = ["pop1", "pop2", "pop3", "R1", "R1a", "R2", "R3", "R3a", "O"]
demography.set_symmetric_migration_rate( populations = allpopulations, rate=0.005 )
...
demography.add_instantaneous_bottleneck(time=600, strength=args.strength, population="R1a")
